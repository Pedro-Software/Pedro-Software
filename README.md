<!-- HEADER WAVE -->
<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,40:0f2040,100:0d1117&height=210&section=header&text=Pedro%20Henrique&fontSize=54&fontColor=58a6ff&fontAlignY=40&desc=Engenharia%20de%20Software%20%C2%B7%20IFPE&descSize=15&descAlignY=60&descColor=8b949e&animation=fadeIn" width="100%" />

</div>

<!-- TYPING ANIMATION -->
<div align="center">

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=700&size=22&duration=2600&pause=1000&color=58A6FF&center=true&vCenter=true&width=680&lines=Hi%2C+I'm+Pedro+Henrique+%F0%9F%91%8B;Cybersecurity+Analyst+%7C+Blue+Team;DevSecOps+%26+Infrastructure+Engineer;Software+Engineering+Student+%40+IFPE;Builder+of+things+that+actually+work." alt="Typing SVG" />
</a>

<br/>

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0a66c2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/pedrohenrique-devsecops)
[![GitHub](https://img.shields.io/badge/-GitHub-161b22?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Pedro-Software)
[![Email](https://img.shields.io/badge/-Email-ea4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:engpedro.run@gmail.com)

</div>

---

## Sobre mim

Estudante de Engenharia de Software no IFPE, com foco em construir coisas que funcionam de verdade — não apenas protótipos.

Meu interesse está na intersecção entre **segurança**, **infraestrutura** e **desenvolvimento**: entender como sistemas falham para construí-los melhores. Trabalho com Linux no dia a dia, uso Docker em praticamente tudo e dedico tempo crescente a práticas de DevSecOps e monitoramento de ambientes.

Paralelamente, exploro dados e IA não como hype, mas como ferramentas concretas para resolver problemas reais.

---

## Áreas de atuação

<table>
<tr>
<td width="50%" valign="top">

**🔐 Segurança & DevSecOps**

Arquitetura SOC, monitoramento contínuo, análise de logs e resposta a incidentes. Hardening, SIEM e integração de segurança no ciclo de desenvolvimento.

</td>
<td width="50%" valign="top">

**⚙️ Infraestrutura & Automação**

Linux no dia a dia, containerização com Docker, automação de pipelines e boas práticas de CI/CD. Do ambiente ao deploy de forma controlada.

</td>
</tr>
<tr>
<td width="50%" valign="top">

**🌐 Desenvolvimento Web**

Aplicações web orientadas ao produto, APIs e integrações. Foco em usabilidade real e arquitetura que escala.

</td>
<td width="50%" valign="top">

**📊 Dados & Inteligência Artificial**

Machine learning aplicado, análise e visualização de dados. MLOps e integração de modelos em ambientes de produção.

</td>
</tr>
</table>

---

## Tecnologias

<div align="center">

<img src="https://skillicons.dev/icons?i=python,linux,bash,docker,git,githubactions&theme=dark&perline=6" />

<br/><br/>

<img src="https://skillicons.dev/icons?i=html,css,js,nodejs,grafana,elasticsearch&theme=dark&perline=6" />

<br/><br/>

<img src="https://skillicons.dev/icons?i=sklearn,tensorflow,kubernetes,terraform,ansible,nginx&theme=dark&perline=6" />

</div>

<br/>

<div align="center">

![](https://img.shields.io/badge/Blue_Team-Defender-161b22?style=flat-square&color=1f6feb)
![](https://img.shields.io/badge/SOC-Architecture-161b22?style=flat-square&color=58a6ff)
![](https://img.shields.io/badge/SIEM-ELK_Stack-161b22?style=flat-square&color=3fb950)
![](https://img.shields.io/badge/Hardening-Linux-161b22?style=flat-square&color=f78166)
![](https://img.shields.io/badge/CI%2FCD-DevSecOps-161b22?style=flat-square&color=a371f7)
![](https://img.shields.io/badge/Incident-Response-161b22?style=flat-square&color=1f6feb)
![](https://img.shields.io/badge/Threat-Intelligence-161b22?style=flat-square&color=f78166)
![](https://img.shields.io/badge/Bash-Scripting-161b22?style=flat-square&color=a371f7)
![](https://img.shields.io/badge/ML-scikit--learn-161b22?style=flat-square&color=3fb950)
![](https://img.shields.io/badge/Docker-Containers-161b22?style=flat-square&color=58a6ff)

</div>

---

## Projetos em destaque

<table>
<tr>
<td width="50%" valign="top">

### 🛡️ [Arquitetura SOC](https://github.com/Pedro-Software/Arquitetura---SOC)

Arquitetura completa para um Centro de Operações de Segurança — monitoramento contínuo, correlação de eventos, SIEM open source e fluxo de resposta a incidentes.

`ELK` `Grafana` `Wazuh` `Linux` `Blue Team`

</td>
<td width="50%" valign="top">

### 🚀 [AstroPose — NASA Hackathon 2025](https://github.com/Pedro-Software/AstroPose---HackathonNasa---2025)

Desenvolvido no Space Apps Challenge da NASA. Tecnologia interativa aplicada a um problema espacial real — ideado, prototipado e entregue sob pressão de tempo.

`Python` `Computer Vision` `Hackathon` `NASA`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🛒 [Marketplace Moura Project](https://github.com/Pedro-Software/Marketplace-Moura-Project)

Plataforma de marketplace com foco em produto e experiência do usuário. Arquitetura web completa com atenção real à usabilidade.

`JavaScript` `HTML/CSS` `Full Stack` `UX`

</td>
<td width="50%" valign="top">

### 🌦️ [Weather Decision Tree](https://github.com/Pedro-Software/weather-decision-tree)

Classificação climática com árvore de decisão aplicada em dados meteorológicos reais — do pré-processamento ao modelo final.

`Python` `scikit-learn` `Pandas` `Machine Learning`

</td>
</tr>
</table>

---

## Como penso tecnologia

> Prefiro entender profundamente um sistema a decorar sua sintaxe.

> Segurança não é camada — é mentalidade que permeia todo o processo de construção.

> Automação existe para liberar tempo para pensar, não para substituir o pensamento.

---

## Atualmente estudando

<div align="center">

![](https://img.shields.io/badge/Cibersegurança-Blue_Team_%26_Threat_Analysis-0d1117?style=flat-square&labelColor=161b22&color=58a6ff)
![](https://img.shields.io/badge/Kubernetes-Orquestração_de_Containers-0d1117?style=flat-square&labelColor=161b22&color=3fb950)
![](https://img.shields.io/badge/MLOps-Modelos_em_Produção-0d1117?style=flat-square&labelColor=161b22&color=a371f7)
![](https://img.shields.io/badge/CI%2FCD_Seguro-Security_Gates-0d1117?style=flat-square&labelColor=161b22&color=f78166)
![](https://img.shields.io/badge/Threat_Hunting-Detecção_Proativa-0d1117?style=flat-square&labelColor=161b22&color=1f6feb)

</div>

---

## Contribuições

<div align="center">

<img src="https://raw.githubusercontent.com/Pedro-Software/Pedro-Software/output/github-contribution-grid-snake-dark.svg" alt="Contribution Snake" />

</div>

---

<!-- FOOTER WAVE -->
<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,40:0f2040,100:0d1117&height=130&section=footer&animation=fadeIn" width="100%"/>

**`engpedro.run@gmail.com`** &nbsp;·&nbsp; [LinkedIn](https://www.linkedin.com/in/pedrohenrique-devsecops) &nbsp;·&nbsp; [GitHub](https://github.com/Pedro-Software)

<sub>Construindo com consistência.</sub>

</div>
